...
位置：`C:\Users\Administrator\.claude\cc-toast\cc-toast.ps1`，单文件，仅依赖 Windows 自带 .NET WinForms。

- 无边框窗口，`TopMost = $true`
- 位置：主屏 `WorkingArea` 右下角，边距 12px；已有 toast 时向上堆叠（按现存 toast 窗口数偏移 Y）
- `ShowWithoutActivation`：弹出不抢焦点
...
1. Hook 触发时脚本顺父进程链回溯，找到有主窗口句柄的祖先进程（VSCode / Windows Terminal 等），把 PID 传给弹窗
2. 点击时 Win32 `ShowWindow(SW_RESTORE)` + `SetForegroundWindow` 置前
3. 兜底：PID 失效时激活最近的 VSCode / Windows Terminal 主窗口；再失败则仅关闭 toast
...
3. 点击跳转：从 VSCode 启动，验证点击后 VSCode 置前
